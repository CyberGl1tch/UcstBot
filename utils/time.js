...
module.exports.EuropeTimeOnly = EuropeTimeOnly
...
function EuropeTimeOnly(){
    let temp1 = new Date().toLocaleString("en-US", {timeZone: "Europe/Athens"});
    temp1 = temp1.replace("AM","").trim()
    let list = temp1.split(",")
    let date = list[0].trim().split("/")
    let hour = list[1].trim().split(":")
    return `${parseInt(hour[0])<10 ? "0"+parseInt(hour[0]) : parseInt(hour[0]) }:${parseInt(hour[1])<10 ? "0"+parseInt(hour[1]) : parseInt(hour[1])}:${parseInt(hour[2])}`
}
